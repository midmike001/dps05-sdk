...
| `PROP_HVAC_POWER_ON` | `0x35400101` | Area 1 (Driver) & Area 0 (Global) | `Int` | Climate system power (2=On, 1=Off) |
| `PROP_HVAC_AC_ON` | `0x35400102` | Area 1 (Driver) | `Int` | AC compressor state (1=On, 2=Off) |
| `PROP_HVAC_AUTO` | `0x35400104` | Area 1 (Driver) & Area 2 | `Int` | Full automatic climate control (1=Auto, 2=Manual) |
...
| `PROP_HVAC_MAX_AC` | `0x3540010b` | Area 1 (Driver) / Area 2 | `Int` | Maximum cooling mode (1=On, 2=Off) |
...
| `PROP_SEAT_MASSAGE_TOGGLE` | `0x31400b2f` | Area 0 (Driver), Area 4 (Passenger) | `Int` | Pneumatic massage toggle (2=On, 1=Off) |
| `PROP_SEAT_MASSAGE_MODE` | `0x31400b30` | Area 0 (Driver), Area 4 (Passenger) | `Int` | Pneumatic massage pattern/mode (1 to 8) |
| `PROP_SEAT_MASSAGE_LEVEL` | `0x31400b31` | Area 0 (Driver), Area 4 (Passenger) | `Int` | Pneumatic massage intensity level (1 to 3) |
...
## 5. Seat Comfort & Pneumatic Massage

### Seat Heating & Ventilation
Both driver (Area 1) and front passenger (Area 4) seats feature independent 3-level heating and ventilation:
- Level `0`: Off
- Level `1`: Low
- Level `2`: Medium
- Level `3`: High
...
suspend fun configureSeats() {
    ...
    // Passenger: Vent Level 3
    client.setSeatVentilation(level = 3, area = DeepalS05Property.AREA_PASSENGER)
}
```

### Pneumatic Seat Massage
- **Toggle (`0x31400b2f`)**: 2 = On, 1 = Off
- **Pattern Mode (`0x31400b30`)**: Modes 1 to 8 (e.g., Wave, Ripple, Lumbar, Shoulder)
- **Intensity Level (`0x31400b31`)**: Levels 1 to 3 (Low, Medium, High)

```kotlin
suspend fun activateDriverMassage() {
    // Enable driver massage with pattern mode 2 and intensity level 3
    client.setSeatMassage(enabled = true, mode = 2, level = 3)
}

suspend fun activatePassengerMassage() {
    // Enable passenger massage on Area 4
    client.setPassengerSeatMassage(enabled = true, mode = 1, level = 2)
}
```
